...
# Queries
expensive_items = (
    """SELECT ProductName FROM Product order by UnitPrice DESC Limit 20;"""
)
avg_hire_age = (
...
ten_most_expensive = """SELECT ProductName, CompanyName
...
most_territories = """select firstname, lastname, max(count_distinct_TerritoryId)
from (select firstname, LastName, count(count_TerritoryId)
as count_distinct_TerritoryId
from (select firstname, LastName, TerritoryId, count(TerritoryId)
